...
object QSTileIconShapes {
    ...
    /**
     * The corner fraction for [key], or null for [DEFAULT_KEY], which leaves every surface at the
     * radii it already draws.
     */
    fun cornerFractionForKey(key: String): Float? = CORNER_FRACTION_BY_KEY[key]

    /**
     * Ceiling for the media card, the one surface large enough in both axes for the fraction to
     * produce a lozenge: 0.50 of a 168dp card is an 84dp corner. It takes the smaller of the two.
     */
    val MEDIA_CORNER_MAX = 24.dp

    /** Every accepted setting value: the silhouettes plus [DEFAULT_KEY]. */
    val ALL_KEYS: Set<String> = PATH_BY_KEY.keys + DEFAULT_KEY

    private val shapeCache = LinkedHashMap<String, Shape>()

    fun isKnownKey(key: String): Boolean = key in ALL_KEYS

    fun pathForKey(key: String): String =
        PATH_BY_KEY[key] ?: PATH_BY_KEY[FALLBACK_PATH_KEY]!!

    /** The silhouette for [key], or null for [DEFAULT_KEY] — "no custom shape, stay dynamic". */
    fun shapeForKey(key: String): Shape? {
        if (key == DEFAULT_KEY) {
            return null
        }
        // The real CircleShape, not the path: consumers key off shape identity (QSTileStyleWrapper).
        if (key == FALLBACK_PATH_KEY) {
            return CircleShape
        }
        shapeCache[key]?.let { return it }

        val pathData = pathForKey(key)
        val shape = PathShape(pathData)
        shapeCache[key] = shape
        return shape
    }

    private class PathShape(pathData: String) : Shape {
        private val basePath: android.graphics.Path = try {
            PathParser.createPathFromPathData(pathData)
        } catch (_: RuntimeException) {
            PathParser.createPathFromPathData(PATH_BY_KEY[FALLBACK_PATH_KEY]!!)
        }
        ...
        override fun createOutline(
            ...
            val scaled = android.graphics.Path(basePath)
            val matrix = Matrix()
...
/**
 * The custom silhouette, for 1x1 tiles and 1x1 control buttons whose cell is 1:1. Null means
 * [QSTileIconShapes.DEFAULT_KEY]: no custom shape, so the consumer keeps the AOSP state morph.
 */
val LocalQSTileShape = compositionLocalOf<Shape?> { null }

/**
 * How round every surface that cannot carry a silhouette should be, as a fraction of its own size.
 * Null keeps the radii those surfaces already draw, the tiles' active/inactive morph included.
 */
val LocalQSTileCornerFraction = compositionLocalOf<Float?> { null }

/**
 * A rounded-corner shape whose radius is [fraction] of the surface's shorter side. The shorter
 * side is deliberate: tiles in a row share a height, so they share a radius. On a 1:1 cell this
 * reproduces what the silhouette's path draws, and 0.5 lands on a stadium with no clamping.
 */
fun proportionalCornerShape(fraction: Float): RoundedCornerShape =
    RoundedCornerShape(ProportionalCornerSize(fraction))

/**
 * [proportionalCornerShape] with a ceiling. Applied in the [CornerSize], where the surface's real
 * size is known.
 */
fun cappedProportionalCornerShape(fraction: Float, max: Dp): RoundedCornerShape =
    RoundedCornerShape(ProportionalCornerSize(fraction, max))

private data class ProportionalCornerSize(
    private val fraction: Float,
    private val max: Dp? = null,
) : CornerSize {
    override fun toPx(shapeSize: Size, density: Density): Float {
        if (!shapeSize.isSpecified || shapeSize.width <= 0f || shapeSize.height <= 0f) {
            return 0f
        }
        val proportional = min(shapeSize.width, shapeSize.height) * fraction
        val ceiling = max?.let { with(density) { it.toPx() } } ?: return proportional
        return min(proportional, ceiling)
    }
}

/** Reads [Settings.System.QS_TILE_ICON_SHAPE] and tracks changes to it. */
@Composable
fun rememberQSTileShapeKey(): String {
    ...
    fun readKey(): String {
        ...
        return stored?.takeIf { QSTileIconShapes.isKnownKey(it) } ?: QSTileIconShapes.DEFAULT_KEY
    }

    var key by remember { mutableStateOf(readKey()) }

    DisposableEffect(contentResolver) {
        val observer =
            object : ContentObserver(null) {
                override fun onChange(selfChange: Boolean) {
                    context.mainExecutor.execute { key = readKey() }
                }
            }
...
/** The selected silhouette, or null for the default. */
@Composable
fun rememberQSTileShape(): Shape? {
    val key = rememberQSTileShapeKey()
    return remember(key) { QSTileIconShapes.shapeForKey(key) }
}
